...
with st.sidebar:
    ...
    model = st.selectbox("Modelo Gemini", ["gemini-3.5-flash-lite", "gemini-2.5-flash"], index=0)
    st.divider()
...
st.subheader("1. Ingresar glosa")
entrada = st.radio(
    "¿Cómo tienes la glosa?",
    ["Cargar PDF", "Pegar o escribir texto"],
    horizontal=True,
)
...
if entrada == "Cargar PDF":
    glosa = st.file_uploader(
    ...
    glosa_text = st.text_area(
        "Pega o escribe aquí el texto completo de la glosa",
        height=300,
        placeholder="Pega aquí la glosa, incluyendo número de factura, valor, causal u observaciones y demás información disponible.",
    )
...
if (entrada == "Cargar PDF" and glosa) or (entrada == "Pegar o escribir texto" and clean(glosa_text)):
